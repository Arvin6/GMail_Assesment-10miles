...
class GmailAPI:
    ...
    def get_message_id_list(self, query=None, labels=None):
        ...
        while "nextPageToken" in result and (pages>0):
            request = (self.service.users().messages()
                            .list(userId=userId, labelIds=labels, q=query, maxResults=500, pageToken=result["nextPageToken"]))
    ...
    def get_message_dict_list(self, message_id_list):
        ...
        # Chunking to avoid quota limitations
        message_id_chunks = [message_id_list[message_id:message_id + 100] for message_id in range(0, len(message_id_list), 100)]
        for message_batch in message_id_chunks:
            batch_request = self.service.new_batch_http_request()            
            for m_id in message_batch:
